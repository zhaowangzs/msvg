...
class RefCrowdDataset(CustomDataset):
    CLASSES = ('person',)
    ...
    def load_annotations(self, ann_file):
        ...
        self.label_length = self.info['label_length'] #refer
    ...
    def _parse_ann_info(self, sent_id):
        ...
        参数：
                    ann_info（列表[字典]）：图像的注释信息。
                    with_mask（bool）：是否解析掩码批注。

        返回：
                    dict：包含以下键的字典：bbox、bboxes_ignore、
                        标签、mask,seg_map。“掩码”是原始注释，而不是
                        解码为二进制掩码。
        """

        sent_info=self.Sentences[sent_id]
        ref_category_id=0
        gt_ref_bboxes = xywh_to_xyxy(np.vstack([sent_info['region']])).astype(
            np.float32)
        gt_ref_masks = None
