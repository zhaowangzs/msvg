...
from .BcrossAtten import build_CrossEncoder
...
class VLTVG(nn.Module):
    def __init__(self, pretrained_weights, args=None):
        ...
        self.crossEncoder = build_CrossEncoder(args)
        # visual grounding
    ...
    def forward(self, image, image_mask, word_id, word_mask):
        ...
        Voutput, Toutput = self.crossEncoder (src = img_feat,txt_emb =  word_feat,src_mask = mask,pos=pos_embed[-1])

        # Discriminative feature encoding + Multi-stage reasoning
        #hs = self.trans_decoder(img_feat, mask, pos_embed, word_feat, word_mask)
        hs = self.trans_decoder(img_feat, word_feat, word_id)
