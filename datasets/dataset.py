...
class VGDataset(Dataset):
    def __init__(self, data_root, split_root='data', dataset='referit', transforms=[],
        ...
        #print('self.imgset_info:',self.imgset_info)
        if self.dataset == 'flickr' :
            self.img_names, self.bboxs, self.phrases = zip(*self.imgset_info)
        elif self.dataset == 'rsvg' :
            self.img_names, self.bboxs, self.phrases, _, _ = zip(*self.imgset_info)
        else:
            self.img_names, _, self.bboxs, self.phrases, _ = zip(*self.imgset_info)
        #print('self.img_names:',self.img_names)
        #print('self.bboxs:',self.bboxs)
        ...
        self.covert_bbox = []
        if not (self.dataset == 'referit' or self.dataset == 'flickr' or self.dataset == 'rsvg'):  # for refcoco, etc
            # xywh to xyxy
            ...
            for bbox in self.bboxs:  # for referit, flickr
                bbox = np.array(bbox, dtype=np.float32)
                self.covert_bbox.append(bbox)
        #print('self.covert_bbox:',self.covert_bbox)
