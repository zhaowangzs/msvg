...
class MultiStageDecoderLayer(nn.Module):
    ...
    def forward(self, vis_query, vis_query_pos, text_query_pos,
        ...
        #print('decoder：')
        #print('text_query.size:', text_query.size())
...
class DecoderWithExtraEncoder(nn.Module):
    ...
    def forward(self, img_feat, img_key_padding_mask=None, pos=None,
        ...
        #print('self.text_query_embed.weight.size:', self.text_query_embed.weight.size())
        #print('text_query_embed.size:', text_query_embed.size())
...
class DiscriminativeFeatEncLayer(nn.Module):
    ...
    def forward(self, img_feat, img_key_padding_mask, img_pos,
        ...
        print('img_key_padding_mask.size:', img_key_padding_mask.size())
        print('img_pos.size:', img_pos.size())
        print('word_feat.size:', word_feat.size())
        print('word_key_padding_mask.size:', word_key_padding_mask.size())'''
        img_query = img_feat + img_pos if self.img_query_with_pos else img_feat
        #print('img_query.size:', img_query.size())
        #print('self.with_pos_embed(word_feat, word_pos).size:', self.with_pos_embed(word_feat, word_pos).size())
        text_info = self.img2text_attn(
            query=img_query, key=self.with_pos_embed(word_feat, word_pos),
            value=word_feat, key_padding_mask=word_key_padding_mask)[0]

        #print('text_info.size:', text_info.size())
        text_embed = self.text_proj(text_info)
        #print('text_embed.size:', text_embed.size())
        ...
        #print('verify_score.size:', verify_score.size())
        verify_score = self.tf_scale * \
                       torch.exp( - (1 - verify_score).pow(self.tf_pow) \
                        / (2 * self.tf_sigma**2))
        #print('verify_score2.size:', verify_score.size())
        ...
        text_cond_info = self.img2textcond_attn(
            ...
            value=word_feat, key_padding_mask=word_key_padding_mask)[0]
        #print('text_cond_info.size:', text_cond_info.size())
        ...
        #print('text_cond_img_ctx.size:', text_cond_img_ctx.size())
        # discriminative feature
        fuse_img_feat = (self.norm_img(img_feat) +
                         self.norm_text_cond_img(text_cond_img_ctx)) * verify_score
        #print('fuse_img_feat.size:', fuse_img_feat.size())
        #print('torch.cat([orig_img_feat, fuse_img_feat], dim=-1).size:', torch.cat([orig_img_feat, fuse_img_feat], dim=-1).size())
