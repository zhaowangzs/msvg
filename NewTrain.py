...
from ENomodels import build_model
...
def get_args_parser():
    ...
    # dataset parameters
    parser.add_argument('--data_root', default='/media/xd/disk1/zxh/Mymodel/data/')
    parser.add_argument('--split_root', default='/media/xd/disk1/zxh/Mymodel/data/split/data/')
...
def main(args):
    ...
    #各个模块参数
    backbone_param = [p for n, p in model_without_ddp.named_parameters() if p.requires_grad and ('body' in n) and ('cm_layer' not in n)and('num_batches_tracked' not in n)]
    vis_enc_param = [p for n, p in model_without_ddp.named_parameters() if p.requires_grad and
                     (n.startswith('trans_encoder') or n.startswith('input_proj'))and ('ExtAtten' not in n)]
    ...
    rest_param = [p for n, p in model_without_ddp.named_parameters() if p.requires_grad and
                  (n.startswith('trans_decoder') or n.startswith('bbox_embed') or n.startswith('bert_proj')or ('cm_layer' in n)or('num_batches_tracked' in n) or ('ExtAtten' in n)or ('Attn' in n)) ]
    #计算各个模块参数
    ...
    lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, args.lr_drop)#学习率衰减
    ...
    if args.yaogan:
        ...
        dataset_train = RSVGDataset(images_path=args.images_path,
        ...
        if not args.testyaogan:
            print('train and eval or only val:')
            dataset_val = RSVGDataset(images_path=args.images_path,
                         anno_path=args.anno_path,
                         split = 'val',
                         imsize = args.size,
                         transform=input_transform,
    ...
    elif args.resume_change:
        checkpoint = torch.load(args.resume_change, map_location='cpu')
        model_without_ddp.load_state_dict(checkpoint['model'], strict=False)
        if not args.eval and 'epoch' in checkpoint:
            args.start_epoch = checkpoint['epoch'] + 1
    elif args.load_weights_path:
        print("load from pretrain weights")
        model_without_ddp.load_pretrained_weights(args.load_weights_path)
        if args.freeze_encoder:
            print("only loading encoder from last stage")
            checkpoint = torch.load(args.load, map_location="cpu")
            encoder_dict = {k: v for k, v in checkpoint["model"].items() if 'encoder' in k}
    ...
    if args.start_epoch < args.freeze_epochs and args.freeze_modules:
        ...
        if args.unfreeze_param_names:
            logger.info(f'Unfreeze weights: {args.unfreeze_modules} and {args.unfreeze_param_names}')
            param_unfreezer = ParamFreezer(args.unfreeze_modules, args.unfreeze_param_names)
            param_unfreezer.unfreeze(model_without_ddp)
