...
from .Pbackbone import build_backbone
...
class VLTVG(nn.Module):
    def __init__(self,config, pretrained_weights, args=None):
        """ Initializes the model."""
        super().__init__()

        # Image feature encoder (CNN + Transformer encoder)
        self.backbone = build_backbone(config, args)
        """self.v_feats_size_list = []"""
        #self.v_feats_size = 2048
        #self.cm_layer = CLIM(self.v_feats_size, config.hidden_size, config.clim_embeding_size)
        ...
        #self.txt_proj = nn.Conv2d(self.backbone.num_channels, self.trans_encoder.d_model, kernel_size=1)
        ...
        def load_weights(module, prefix, weights):
            ...
            for k in module_keys:
                ...
                if prefix_k in weights_keys:
                    ...
                    print(f"YESWeights of {k} are not pre-loaded.")
                else:
                    print(f"NONOWeights of {k} are not pre-loaded.")
    ...
    def forward(self, image, image_mask, word_id, word_mask,token_type_ids=None,attention_mask=None,):

        N = image.size(0)

        # Text features
        word_feat, txt_pos = self.bert(word_id, token_type_ids=None, attention_mask=word_mask)
        #print("word_feat",word_feat.size())
        embedding_output = word_feat
        #print("embedding_output",embedding_output.size())
        word_feat = torch.stack(word_feat[-self.bert_output_layers:], 1).mean(1)
        #print("word_feat_1",word_feat.size())
        word_feat = self.bert_proj(word_feat)
        #print("word_feat_2",word_feat.size())
        #Extra attention layer input
        txt_feat = word_feat
        ...
        # Image features
        features, pos = self.backbone(NestedTensor(image, image_mask),embedding_output, word_id, image)
        #masks=features[-1].mask
        #embedding_output = self.embeddings(word_id, token_type_ids)
        #features = self.cm_layer(features[0].tensors, embedding_output, extended_attention_mask_1)
        #features = NestedTensor(features, masks)

        src, mask = features[-1].decompose()
        assert mask is not None
        #print('self.input_proj(src): ',self.input_proj(src).size())
        #print('word_feat: ',word_feat.size())
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_dict):
        ...
        # Convert to absolute coordinates in the original image转换为原始图像中的绝对坐标
...
def build_vgmodel(config, args):
    device = torch.device(args.device)

    model = VLTVG(config, pretrained_weights=args.load_weights_path, args=args)
